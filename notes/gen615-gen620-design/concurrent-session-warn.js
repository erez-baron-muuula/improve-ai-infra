...
// Every clause is load-bearing: without re-pruning, the re-read resurrects pruned entries and
// the file grows forever; with the STALE liveness snapshot, a session born inside this hook's
// own execution window loses its fresh entry; without re-gating, a transiently degraded
// re-derivation would prune every live session's entries.
...
function folderHalf(data, ctx) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (PROTECTED_BASENAMES.has(path.basename(o.file).toLowerCase())) continue;
    if (norm(path.dirname(o.file)) !== norm(folder)) continue;
  ...
  // the OLDEST — the idle-session case that matters most. Reported as its own note rather
  // than riding on a warning that may not exist.
  if (tail.offset > 0 && isFinite(oldestSeen) && oldestSeen > earliest) {
    const capNote = degradedNote('edit-log-tail-capped', ctx, true);
    if (capNote && !hits.size) return capNote;
  }
  ...
  for (let e = hkeys.next(); !e.done; e = hkeys.next()) {
    ...
    const key = ctx.agentId + '|F|' + norm(folder) + '|' + sid;
    const last = st.pairs[key];
