...
function degradedNote(reason, ctx, throttle) {
  ...
  return '**Concurrent-session check degraded.** ' + what +
    ...
    'folder. Two consequences: it is not warning you when it should, and while degraded it is ' +
    'also not registering this session\'s own tickets, so the other sessions are blind to this ' +
    'one too. Flag this to Erez in the "\u{1F4CC} For you" block.';
}
