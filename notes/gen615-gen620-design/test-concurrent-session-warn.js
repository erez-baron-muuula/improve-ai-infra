...
function reset(opts) {
  ...
  if (!opts.noOtherFile) {
    fs.writeFileSync(path.join(SESSIONS, '1002.json'), JSON.stringify({
      pid: process.pid, sessionId: OTHER, cwd: opts.otherCwd || PROJECT,
      startedAt: now - (opts.otherAgeMs || 7200e3), name: 'test-other', kind: 'interactive',
    }));
  }
  if (opts.deadFile) {
...
check('cap binding with no hit in the tail: the cap note appears',
  run(toolCall(target)),
  has('cap'));
