...
// --- findings from the independent review pass ---

// An unreadable session file must NOT be treated as proof that session is gone: pruning on it
// deletes a live session's registrations, which is the loss the hook exists to prevent.
reset({ map: { [OTHER]: { tickets: { 'GEN-777': Date.now() - 60e3 } } } });
fs.writeFileSync(path.join(SESSIONS, '1099.json'), '{ truncated mid-write');
run(prompt('unrelated prompt naming GEN-888'));
check('a session file that fails to parse does not cause pruning',
  mapNow(),
  m => !!m && !!m[OTHER] && typeof m[OTHER].tickets['GEN-777'] === 'number');

// With every file readable, pruning still works — otherwise the fix above would disable it.
reset({ map: { [DEAD]: { tickets: { 'GEN-777': Date.now() - 60e3 } } }, deadFile: true });
run(prompt('unrelated prompt naming GEN-888'));
check('pruning still removes a dead session when every file was readable',
  mapNow(),
  m => !!m && !m[DEAD]);

// A live lock must not be breakable on age alone, or two writers enter the critical section.
reset({});
fs.writeFileSync(path.join(HOOKS, 'session-tickets.json.lock'), String(process.pid));
const oldTime = new Date(Date.now() - 60 * 60e3);
fs.utimesSync(path.join(HOOKS, 'session-tickets.json.lock'), oldTime, oldTime);
run(prompt('GEN-615 while a live holder holds an old lock'));
check('an old lock held by a LIVE pid is not broken (registration is skipped, not raced)',
  mapNow(),
  m => m === null);
check('  ...and the skipped registration is logged, not silent',
  fs.existsSync(path.join(HOOKS, 'concurrent-session-warnings.jsonl'))
    ? fs.readFileSync(path.join(HOOKS, 'concurrent-session-warnings.jsonl'), 'utf8')
    : '',
  s => /registration-failed/.test(s));

// A lock left by a dead process must be breakable, or the ticket half wedges permanently.
reset({});
fs.writeFileSync(path.join(HOOKS, 'session-tickets.json.lock'), '4194303');
run(prompt('GEN-615 after a dead holder left a lock'));
check('a lock left by a dead pid is broken and the write proceeds',
  mapNow(),
  m => !!m && !!m[SELF] && typeof m[SELF].tickets['GEN-615'] === 'number');

// A relative file_path must resolve against the payload cwd, as the log writer does.
reset({ editLog: editEntry(OTHER, theirs, 4 * 60e3) + '\n' });
check('a relative file_path resolves against the payload cwd',
  run(toolCall(path.join('notes', 'mine.md'))),
  has('theirs.md'));

// A crafted filename must arrive as a label, not as markup in model context.
reset({ editLog: editEntry(OTHER, path.join(SUBDIR, 'x`**evil**`.md'), 4 * 60e3) + '\n' });
check('a filename carrying markup is neutralised before injection',
  run(toolCall(target)),
  got => typeof got === 'string' && got.indexOf('**evil**') === -1 && got.indexOf('evil') !== -1);
