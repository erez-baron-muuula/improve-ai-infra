...
binding" (§5 below). Everything else there — the `/vet-ticket` flow, the summary-card approval, the
waive lane, the unresolved-page hard-block stance, the batch-pass shape — is unchanged and still
governs.
...
`ok = false` also when: the input is neither object, array, nor string; a string that *looks* like
JSON (first non-space char is `{` or `[`) fails to parse; or an `__unparsedToolInput.len` exceeds
its `raw.length` — proof the harness truncated the payload, true in all 3 captured cases.
...
Exempt iff **all** hold on the normalised root `R`:
...
4. Every value inside `R.properties` is a primitive, or an array of primitives. No nested object —
   which is what a content structure looks like.
...
edit on GEN-58 still goes through the gate. Erez's call — §6.
...
1. **Match on `contentHash` alone**, within the ticket-pass dir, with `kind: "ticket"`. One tool call
   ...
   list.
2. **Hash the NORMALISED payload, not the raw `tool_input`** —
   ...
   the only way out. Normalising first makes the two forms hash identically.
...
1. **The GEN-58 carve-out shape** (§3, finding 5) — exempt all content writes to that one log page,
   versus a third attempt at characterising "append". Recommendation: exempt the page's content
   writes. Two semantic rules have now been falsified against real traffic (5% and 15% coverage of
   159 real writes), the page is a log rather than a ticket, and the cost of getting it wrong is a
   hard block on a write a standing rule mandates be immediate.
