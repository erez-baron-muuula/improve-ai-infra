...
> # ⛔ NO LONGER NORMATIVE — retired 2026-08-04
>
> **`design-converged.md` (v6) is the single normative document.** It merged this one in. Do not
> build from anything below.
>
> Three `/check` rounds on the two-document split ended without converging, and all three round-3
> lenses reached the same diagnosis: **the split itself was the defect** — corrections landed
> correctly where written and went stale where a builder would read them. The merge is the fix.
>
> **What here is superseded:** §2 (the whole four-stage algorithm — stage 4's network resolver, its
> budgets and its `unknown` stance are **deleted**, not moved), §4's costs, §5's pass-matching rule,
> §6's decision framing, and §7's round-by-round history.
>
> **Why the resolver is gone.** Erez asked on 2026-08-03 whether every Notion page could simply be
> treated as a ticket, and then why a local ticket-id list would not be updated on every run that
> creates one. The first question was answered by measurement (`measurement-edit-targets.md`: about
> one over-gated write in a thousand); the second showed that a list of what *is* a ticket does no
> work at all, because an unknown page is gated anyway — only a gating-**removing** fact is worth
> holding. What survives is a small exemption list, and the hook now makes **no network call on any
> path**.
>
> **What this file is still good for, and is still cited by v6:** §1's corpus evidence and payload-shape
> table (the reason normalisation is shape-independent), §1's ten-tool surface enumeration, and §3
> finding 5's measurement of where the GEN-58 log actually lives. Those are evidence, not rules, and
> they did not change.
>
> **One correction to a claim below**, recorded because v6 depends on the corrected version: §3
> finding 5 concludes that the volume pages "are out of scope already — no carve-out needed, and none
> of the gate's protection ever applied". That was true only under the resolver design. Under v6 the
> volumes are **in** scope, so the exemption in v6 §5 is what protects them — for the first time — and
> its closed shape refuses `replace_content` and `allow_deleting_content` at zero measured cost.
...
## 6. Decisions this needed from Erez — settled 2026-08-03

1. **SETTLED: exempt all content writes in the GEN-58 subtree** (Erez, 2026-08-03: "I choose A").
   The alternative was a third attempt at characterising "append". Two semantic
   ...
   what they claimed. GEN-453 was extended on 2026-08-03 to also cover a write that lands correctly
   while destroying adjacent content, and raised to Urgent / Highest 🔥 as the ticket that runs
   immediately after GEN-508 ships.
