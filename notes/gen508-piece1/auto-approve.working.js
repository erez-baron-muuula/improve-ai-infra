...
const TICKET_EXEMPT_ROOT_KEYS = new Set([
  'page_id', 'pageId', 'id', 'command', 'properties', 'icon', 'cover', 'is_skill', 'allow_async',
  'content', 'old_str', 'new_str', 'selection_with_ellipsis', 'position'
]);
...
//  4  `new_str`, where present, is not empty or whitespace-only. Costs 1 write in 273 (0.37%) -- one
//     real write legitimately empties a 6,138-character block -- so this is a genuine cost, not a
//     free win, and its escape is one /vet-ticket run. (A clause refusing a merely SHORTER new_str
//     was considered and rejected: shrinking is ordinary editing here.)
...
function ticketIsGen58Exempt(norm, pageIds, exempt) {
  ...
  if (Object.prototype.hasOwnProperty.call(R, 'new_str')) {                       // 4
    const ns = R.new_str;
    if (typeof ns !== 'string' || ns.trim() === '') return false;
  }
...
function ticketScope(tool, ti) {
  ...
  // stableStringify. This matters: an earlier version emitted hash '' on an unreadable payload, so
  // NO record could ever match and break-glass was the only door.
  const hash = norm.ok
...
function ticketSessionDir(transcriptPath) {
  ...
  return transcriptPath.replace(/\.jsonl$/i, '');
}
...
// Search ONLY assistant-authored content; collect every occurrence of the prefix in it; require at
// least one, and require THE LAST ONE to read exactly `PASS <contentHash>`. Last-occurrence defeats
// the self-explaining-reviewer case: a reviewer may mention the form, but its verdict is what it ends
// on. A cap-exceeded transcript is a DISTINCT diagnosis from a missing token and must not be logged
// as one.
function ticketTokenVerdict(sessionDir, agentId, hash) {
  ...
  for (const line of text.split('\n')) {
    ...
    // Re-serialise the assistant record and scan THAT: the prefix contains no character JSON
    // escapes, so it survives stringification intact, and this reads every content block without
    // having to know the block schema.
    const hay = JSON.stringify(rec);
    let from = 0;
    for (;;) {
      const at = hay.indexOf(TICKET_TOKEN_PREFIX, from);
      if (at === -1) break;
      count++;
      last = hay.slice(at + TICKET_TOKEN_PREFIX.length, at + TICKET_TOKEN_PREFIX.length + 128);
      from = at + TICKET_TOKEN_PREFIX.length;
    }
  }
  if (count === 0 || last === null) return 'no-token';
...
function blockTicketVetting(sc) {
  ...
  if (reason === 'internal-error') {
    ...
    why = ' The named reviewer\'s own transcript does not END on' +
          ' "TICKET-REVIEW-VERDICT: PASS <hash>" for THIS content. A record is not evidence; the' +
          ' reviewer\'s last assistant-authored verdict is. If the content changed after the review,' +
          ' re-review it -- reviewed-then-edited content has not been reviewed.';
    ...
    why = ' The GEN-58 exemption list is over its ' + TICKET_EXEMPT_CAP + '-id cap. Nothing prunes it' +
          ' automatically, and reaching the cap means something is appending ids that are not log' +
          ' volumes -- find that bug rather than trimming the list.';
...
// by both, removes the failure mode instead of documenting it.
//
...
function ticketHashCli(argv) {
  ...
  // ok === false is NOT an error here: the hook hashes the raw input in that case, so a payload we
  // could not fully read is still a payload a review can be bound to. Exiting non-zero would leave
  // break-glass as the only door, which was a reviewed defect.
  const h = norm.ok
    ? sha256Hex(stableStringify(norm.root === undefined ? null : norm.root))
    : sha256Hex(stableStringify(ti === undefined ? null : ti));
  if (!norm.ok) {
