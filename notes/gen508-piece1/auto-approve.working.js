...
function ticketDeliveredText(rec) {
  ...
  if (typeof c === 'string') return c;                 // older single-string form: all of it delivered
...
//
// "Final message", not "last occurrence in the file", which is what this did before. The refusal text
// has always said the transcript must "END on" the token, and the design comment above says a
// reviewer's "verdict is what it ends on"; last-in-file did not mean that. It let a reviewer emit PASS
// early and then wander -- or, with the block filter above, put its real verdict in an earlier message
// and something else last. Message granularity is what the skill actually instructs: end your reply
// with the token. Now the code, the comment and the refusal text all say the same thing.
//
