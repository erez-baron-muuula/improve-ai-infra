...
# Part 3 — pricing the housekeeping exemption (measured 2026-08-04)

**Why this exists.** A round-1 review lens found that the design's housekeeping exemption listed
**ten** Team-Tasks properties while Erez's settled decision names three categories —
"status / labels / assignee". Seven fields had been added by my own judgment and never surfaced to
him as the judgment call they were. The lens singled out `Reason`, whose name suggests free text,
and correctly refused to accept its classification as verified.

**Method.** Two sources, both live. (1) The data-source schema, fetched 2026-08-04 via
`notion-fetch` on `collection://bd2cd17b-f58f-4993-8b95-468e881272fa`, which gives every property's
**type**. (2) `measure-housekeeping.js` over the same transcript corpus: deduped
`notion-update-page` payloads, normalised and hoisted as the design specifies, restricted to
`command === 'update_properties'` with a `properties` object, with each key reduced to its base name
by stripping a leading `date:` / `place:` / `userDefined:` qualifier and a trailing
`:start` / `:end` / `:is_datetime`.

## The types settle five of the seven

| property | live type | can it carry prose? |
|---|---|---|
| `Reason` | `select`, options `not defined` / `Event Pending` / `Ticket Pending` | **no** — three workflow values |
| `Project` | `select`, one option (`AI`) | no |
| `Due Date` | `date` | no |
| `Remind me (days before)` | `number` | no |
| `Date Created` | `created_time` — system-managed | no, and not writable |
| `ID` | `auto_increment_id` — system-managed | no, and not writable |
| `Parent item` | `relation`, limit 1 | no, but it **re-parents a page** |

So the lens's specific worry about `Reason` is **refuted by the schema**, not by argument: it is a
three-option workflow select and cannot carry substance.

## The traffic settles the rest — and the exemption should shrink, not be defended

414 of 1,071 deduped `notion-update-page` payloads are `update_properties` with a properties object.

| exemption set | payloads exempt | share of property updates |
|---|---|---|
| the design's 10-field list | 201 | 48.6% |
| Erez's literal status / labels / assignee | 186 | 44.9% |
| the 10-field list with `Parent item` moved to substance | 188 | 45.4% |

Per-field frequency across those 414 payloads, base names:

| field | payloads | classification |
|---|---|---|
| `Status` | 193 | exempt |
| `Priority` | 168 | substance |
| `Gain ratio` | 165 | substance |
| **`Importance`** | **164** | substance — **not in the current schema at all** |
| `Name` | 25 | substance |
| `Parent item` | 14 | exempt today |
| `Assignee` | 10 | exempt |
| `Related to Team-Tasks (Parent item)` | 4 | substance |
| `Reason` | 2 | exempt |
| `title`, `Gain Ratio` (capital R) | 2 each | substance |
| `Related to GEN-337 test DB (Parent item)`, `in_trash`, `Urgency` | 1 each | substance |
| `Due Date`, `Remind me (days before)`, `Date Created`, `ID`, `Type`, `Project` | **0** | exempt today |

Four conclusions, each of which changes the design rather than justifying it:

1. **Six of the ten exempted fields carry zero traffic** — `Due Date`, `Remind me (days before)`,
   `Date Created`, `ID`, `Type`, `Project`. That is exempt surface with no traffic behind it, which is
   the exact argument that retired the `update_verification` exemption. The four that Erez did not
   name (`Due Date`, `Remind me`, `Date Created`, `ID`) should be **dropped**; `Type` and `Project`
   stay only because Erez named them ("labels") and they cost nothing either way.
2. **The seven contested fields buy 15 payloads** (201 − 186 = 3.6% of property updates). The
   extension was never worth defending.
3. **`Parent item` should move to substance**, and it costs **13 payloads** (3.1%). It is one half of
   a relation whose other half (`Children`) the design already calls substance — an indefensible
   asymmetry, since the same graph edge written from either end has the same effect — and there is a
   recorded destructive-op incident class for writing it. Of the 13, eleven are `Parent item` alone
   and two are `Parent item` + `Status`.
4. **The property set has already rotated, and it rotated safely.** `Importance` appears in 164 real
   payloads and is absent from the current schema, where `Urgency` now sits; `Gain Ratio` (capital R)
   and `title` are further spelling variants. Every one of them is treated as **substance** under a
   deny-list, i.e. more gating — which is the design's stated claim about rotation direction, now
   demonstrated on 164 payloads rather than asserted.

**Net effect on the design:** the housekeeping list shrinks from ten fields to five — `Status`,
`Assignee`, `Type`, `Project`, `Reason` — of which Erez named four and the fifth is settled by its
type. The exemption covers 45.4% → **188 of 414** property updates instead of 201, and no judgment
call needs to go to Erez at all.

---

# Part 4 — pricing the raw REST/curl hole (measured 2026-08-04)

**Why this exists.** The design admits that raw REST/curl Notion writes are out of scope for piece 1
and "not gated at all", and calls it "an open hole, not a cushioned one". A round-1 review lens noted
that this was the only scoping boundary in the whole design left **qualitative** while every other one
was quantified from the same corpus, and that measuring it was cheap. It was, and the number changes
the picture rather than confirming it.

**Method.** `measure-rawrest.js`: scan every `tool_use` block for the `Bash` and `PowerShell` tools,
dedupe on `tool + command`, keep the commands whose text contains `api.notion.com`, then split them by
HTTP method and by the `/v1/<segment>` they address. Tokens are redacted from any printed sample.

```
distinct shell commands scanned: 7338
   of those, touching api.notion.com: 1227
```

| bucket | calls | read or write |
|---|---|---|
| `POST /v1/databases/query` | 409 | read (a query) |
| `GET /v1/blocks` | 309 | read |
| `GET /v1/pages` | 243 | read |
| **`PATCH /v1/blocks`** | **100** | **write** |
| **`PATCH /v1/pages`** | **87** | **write** |
| `GET /v1/databases` | 25 | read |
| `POST`/body-bearing to `/v1/blocks` with no explicit `-X` | 17 | probably write |
| **`POST /v1/pages`** | **11** | **write — a page create** |
| body-bearing to `/v1/pages` with no explicit `-X` | 10 | probably write |
| **`DELETE /v1/blocks`** | **6** | **write — destructive** |
| body-bearing to `/v1/databases` | 2 | probably write |
| `GET /v1/users` | 2 | read |
| **`DELETE /v1/pages`**, **`PATCH /v1/databases`** | 1 each | **write — destructive / schema** |
| `POST /v1/search`, `POST /v1/comments`, `POST /v1/pages/query`, `GET /v1/?` | 1 each | read / comment |

**Totals: 195 unambiguous writes** (`PATCH` 188, `DELETE` 7) **plus ~38 probable** (the body-bearing
calls with no explicit method, plus `POST /v1/pages`) — call it **~230 raw REST write calls**.

**For comparison, the MCP path in the same corpus** carries 1,067 deduped `notion-update-page` and 263
`notion-create-pages` payloads = 1,330. So raw REST/curl is on the order of **15% of all Notion write
traffic** (230 / 1,560).

Three of those buckets are exactly the traffic GEN-508 exists to govern:

- **`PATCH /v1/pages` (87)** is a page property or content update — the same operation as
  `notion-update-page`. Property updates through this path include the priority-derivation fields,
  which are the headline case in GEN-508's problem statement.
- **`POST /v1/pages` (11)** is a page **create** — the same operation as `notion-create-pages`.
- **`DELETE /v1/blocks` (6)** and **`DELETE /v1/pages` (1)** are destructive and have no MCP equivalent
  in the gated set at all.

## Part 4a — do raw-REST writes hit the same pages the MCP path edits? (measured 2026-08-04)

**Why this exists.** Round 2 produced a third option for the raw-REST hole — *deny raw REST writes and
funnel everything through the gated MCP tools* — which two lenses raised independently. That option is
only viable if the writes have an MCP equivalent, so: do they target pages the MCP tools also write?

**Method.** `measure-rest-targets.js` re-detects raw-REST writes using the *same* rule
`notion-schema-guard.js` uses (`isNotionMutatingHttp`: an explicit `PUT`/`PATCH`/`DELETE`, or a
`POST`/body-bearing call, minus a read allow-list for `/query` and `/v1/search`), extracts the id
immediately after `/v1/<segment>/`, and intersects it with the 318 distinct MCP edit-target ids from
Part 1.

| bucket | calls | with an id in the URL | distinct ids | calls whose target the MCP path also edits |
|---|---|---|---|---|
| `PATCH /v1/blocks` | 101 | 46 | 21 | 25 |
| `PATCH /v1/pages` | 88 | 27 | 18 | 19 |
| `POST`/body `/v1/pages` | 21 | 8 | 7 | 5 |
| `POST`/body `/v1/blocks` | 17 | 16 | 5 | 15 |
| `DELETE /v1/blocks` | 5 | 3 | 3 | **0** |
| `POST`/body `/v1/databases` | 2 | 2 | 1 | 0 |
| `PATCH /v1/databases` | 1 | 0 | 0 | 0 |
| `POST`/body `/v1/comments` | 1 | 0 | 0 | 0 |

**236 write commands; 46 distinct target ids, of which 25 are also edited through the MCP tools.**

What this supports and what it does not:

- **For page-targeted writes the funnel works.** `PATCH /v1/pages` (19 of 27 identifiable calls) and
  `POST /v1/pages` (5 of 8) hit pages the MCP tools already edit, so an MCP equivalent demonstrably
  exists for them.
- **Most `/v1/blocks` writes are page-content edits in disguise.** 25 of 46 identifiable
  `PATCH /v1/blocks` calls and 15 of 16 `POST /v1/blocks` calls carry an id that is in the *page*
  target set — which is expected, because in Notion a page **is** a block, so
  `PATCH /v1/blocks/{page-id}/children` is the append-children endpoint. Those map onto
  `notion-update-page`'s `insert_content` / `update_content`.
- **`DELETE /v1/blocks` is the genuine exception: 0 of 3.** Deleting an individual block has no direct
  MCP equivalent short of `replace_content` with `allow_deleting_content`. Same for the two
  `/v1/databases` writes. These are the calls a funnel would strand.
- **Only 102 of 236 commands put the id in the URL at all** — the rest interpolate a shell variable or
  send a body from a temp file. So the overlap is measured on a 43% subset and is indicative, not
  exhaustive.

**Reconciliation with Part 4's counts.** Part 4's script bucketed 195 unambiguous + ~38 probable writes
(~233); this one, using the schema-guard rule, matched **236**. The ~1% spread comes from the two
scripts' different write-detection rules, not from different data. Both are the same order and neither
is a precise call count.

## Limits on Part 4

- **Method detection is textual.** A `curl` that sets its method by another route, or a Node/Python
  script that makes the call without the URL appearing in the shell command, is not counted. So the
  totals are a **floor**, not a ceiling.
- **Deduping on the exact command string** means near-identical calls differing only by page id count
  separately — which is the right unit here (each is a distinct write) but is not comparable to
  Parts 1–3's deduped-payload unit without care.
- **No attempt was made to classify the target** as a Team-Tasks row versus another page, which would
  need 230 page fetches. So "15% of write traffic" is not the same as "15% of *ticket* writes"; it
  bounds the surface, not the ticket-relevant subset.
- **Not every one of these is ticket authoring.** Some are investigation or repair commands issued
  while debugging. They are still ungated writes to the same pages.

<details><summary><code>measure-rawrest.js</code></summary>

Walk transcripts to depth 8; keep `Bash`/`PowerShell` `tool_use` blocks whose `command` contains
`api.notion.com`; dedupe on tool + command; bucket by explicit `-X`/`--request` method, by whether a
body flag is present, and by the `/v1/<segment>` addressed; redact `Bearer` tokens from samples.

</details>

## Limits on Part 3

- Same deduped-payload unit as Parts 1 and 2.
- `baseName` strips only the qualifier forms the design names. `Related to Team-Tasks (Parent item)`
  therefore does **not** reduce to `Parent item` and is counted as substance — which is also how the
  design's matcher will treat it, so the count and the behaviour agree.
- **Zero traffic is not zero probability.** A future pure-`Due Date` edit will be gated once these
  four fields are dropped. The cost is one review, and it fails in the safe direction.
- **`Parent item` appears in 14 payloads but gating it costs 13** (round-2 flag, reconciled here): the
  fourteenth also carries a substance key, so it was already gated and moving `Parent item` changes
  nothing for it. The 13 are the payloads that were exempt *only* because `Parent item` was on the list.
- **"Labels" → `Type` + `Project` is an interpretation, not a measured match.** Team-Tasks has no
  property literally named "Labels", so mapping Erez's word onto those two selects is my reading. It is
  the one interpretive step left in the list, and it costs nothing either way because both fields carry
  zero traffic.
- **Relation properties, verified rather than assumed** (a round-1 advisory that stayed open through
  round 2): the schema fetch returns 17 properties, and exactly two are `type: relation` — `Parent item`
  (limit 1) and `Children`. So the design's "exactly two relation properties" claim holds.

<details><summary><code>measure-housekeeping.js</code></summary>

Walk transcripts to depth 8; dedupe `notion-update-page` payloads; normalise + hoist; keep
`update_properties` payloads with a `properties` object; reduce each property key to its base name;
then evaluate three candidate exemption sets against the design's root-key and value-shape clauses
and tally per-field frequency.
