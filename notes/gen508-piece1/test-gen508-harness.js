...
// The fail-open corpus of REAL payloads (built by build-corpus.js). It holds real ticket bodies, so it
// lives OUTSIDE the repo -- a stable temp path, NOT a session-scoped scratchpad (the old build-corpus
// hardcoded a stale session UUID no reader used) and NOT beside the script where it could be committed.
// build-corpus.js writes here; the arm suite's corpus sweep reads here, skipping with a notice when it
// is absent. One definition, imported by both, so they can never look in different places.
const CORPUS = path.join(os.tmpdir(), 'gen508-notion-payload-corpus.jsonl');
...
const TICKET_BLOCK_SIGNATURES = {
  'internal-error': 'internal error while working out whether',
...
module.exports = {
  DIR: DIR, HOOK: HOOK, SCRIPT: SCRIPT, PASS_DIR: PASS_DIR, CORPUS: CORPUS,
