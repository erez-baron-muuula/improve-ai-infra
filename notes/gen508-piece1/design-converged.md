# GEN-508 piece 1 — design (v6.1, the single normative document)
...
- **~230 raw REST write calls** in the corpus — 195 unambiguous (`PATCH` 188, `DELETE` 7) plus ~38
  probable — against 1,330 MCP write payloads: roughly **15% of all Notion write traffic**. Read that
  as an order of magnitude, not a precise share: the two populations are deduped differently (exact
  command string vs exact payload), and it is a **floor**, since a write whose URL never appears in the
  shell command is not counted. An independent re-detection using `notion-schema-guard`'s own rule
  matched 236, a ~1% spread (Part 4a).
...
started over.** Round 1 on v6: holistic **PASS**, pre-mortem **REVISE** (3 material findings), soundness
**REVISE** (1 material finding) — all four addressed above, and one of them promoted to an open decision
for Erez (§12.1) rather than resolved by me. **v6.1 has not yet been re-reviewed.**
