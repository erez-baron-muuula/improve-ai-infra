...
>
> **Review state: four rounds, ten reviews. See the box directly below.**

> ### v7 `/check` — four rounds, ten reviews
>
...
> (pre-mortem), `a0700fdd0f864ecdc` (soundness). All `check-reviewer`, 2026-08-04; rounds 3–4 on Opus.
>
...
>
> **The class that recurred FOUR times, which is the useful thing here.** Every round found a different way for
...
> — the closed shape — and it is what the standing stop-and-re-derive rule was actually asking for. Round 4 also
> showed the outcome was worse than "ungated": a record was minted, so the write was *falsely assured*.
>
> **Status of the current text: the round-4 fix is applied but not yet re-reviewed.** It was authored by two
> independent reviewers rather than by me, which is a materially better starting point than v6.3's unreviewed
> residue was — but the applied wording is mine, and in this session every one of my applications of a fix has
> been where the next defect appeared. **A round 5 on gate 1's applied text is the honest next step** before any
> code is written from this section. Advisories fixed in the same pass: the token count is scoped to unquoted
...
the second time a "structural" fix of mine turned out to be an enumeration one level up; the closed shape is
what the standing stop-and-re-derive rule was actually asking for.
