...
**This rule is only sound because §9 permits at most one live adjudication per hash.** A single
`adjudication` slot cannot express "finding 1 overturned, finding 2 still open", so if two findings could
be live at once, one OVERTURNED token would clear a record that still carried an unresolved objection.
§9's "exactly one outstanding finding" precondition is what makes the substitution safe; do not implement
this section without it.
...
- **Exactly one adjudication may be live per `contentHash`.** This is the round-2 correction and it is
  load-bearing, not bookkeeping. The record carries a single `adjudication` slot and the hook's check
  asks only "does a qualifying OVERTURNED token exist for this hash". If a review returned **two**
  findings and Claude disputed one and won, nothing in the record or the hook would notice that the
  *other* finding was never fixed, waived or overturned — Claude would be choosing which of the
  reviewer's objections to answer while producing a record that passes every check. That is precisely
  the "judge in its own case" failure adjudication exists to remove, and it was introduced by the fix
  that made OVERTURNED work at all.

  **The rule that closes it:** adjudication may only be invoked when the reviewer's REVISE leaves
  **exactly one** outstanding finding on this hash. Any co-occurring findings must be fixed first — and
  fixing changes the content, so it changes the hash, so the dispute is re-raised against a fresh hash
  in a fresh round. By construction at most one adjudication is ever live per hash, and no finding can
  be silently skipped. The alternative — a list of adjudications plus a hook that verifies a resolution
  per named finding — needs the reviewer's findings to be a structured, machine-readable list rather
  than prose, which is a much larger change for the same guarantee.
- **Bounded.** "Round" means one pass of step 5's review-fix-re-review loop. **At most two adjudications
  per ticket in total** (each on its own hash, per the rule above), not two per round. A finding that
  survives adjudication stands and must be fixed. Past the bound it goes to Erez with both positions
  stated plainly.
...
reason}` — and the hook appends every hard-block with its reason. The complete set: `no-pass`,
`bad-verdict`, `no-token`, `unreadable-payload`, `bad-target`, `exempt-list-overflow`,
`exempt-list-unreadable`, `internal-error`. **Any new block path must add a reason to this list rather
than inherit `internal-error`**, which is reserved for a genuine bug in the arm — the round-2 finding
that produced the two `exempt-list-*` reasons was exactly this omission. Without the log, a
persistently broken gate is indistinguishable from a transient one and shows up only as unexplained
friction.
...
- Two gated surfaces collapse into one: every Notion write must pass through the MCP tools, which the
  rest of this design already governs.
- **Measured viability** (Part 4a): of the identifiable targets, `PATCH /v1/pages` 19-of-27 and
  `POST /v1/pages` 5-of-8 calls hit pages the MCP tools already edit, and 25-of-46 `PATCH /v1/blocks`
  plus 15-of-16 `POST /v1/blocks` calls carry a **page** id — the append-children endpoint — which maps
  onto `insert_content` / `update_content`. So the large majority has a direct MCP equivalent.
- **What it strands, named:** `DELETE /v1/blocks` (5 calls, 0 of 3 identifiable targets in the MCP set),
  `DELETE /v1/pages` (1), and the two `/v1/databases` writes. Those need either break-glass or a narrow
  stated carve-out. Also unmeasured: whether any of the 236 REST writes exist *because* the MCP tool
  could not express the operation — if so, C blocks real work and the carve-out has to grow.
...
**What I would do: Option C**, and this is a change of position from v6.1's first draft, which
recommended A before the existing detector and the §7.1 tension were on the table. C closes the largest
part of the hole, needs the least new machinery, and is the only option that makes the design smaller
rather than larger. Its honest weakness is the stranded destructive verbs, which is a carve-out to write
rather than a reason to keep 15% of writes ungated. **The scope call is Erez's.**
