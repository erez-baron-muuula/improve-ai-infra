...
>
> **Review state: three rounds run, and the panel ESCALATED at its cap. See the box directly below.**

> ### v7 `/check` — three rounds, eight reviews, escalated at the cap
>
...
> `acd5e88a491e4a4f0`; round 3 `a5cc3ab1a784bee78`, `aee729e4666c6470b`. All `check-reviewer`, 2026-08-04.
>
> **The class that recurred three times, because it is the useful thing here.** Every round found a different
> way for a raw-REST write to reach Notion with a body the record's hash did not cover: a parenthesised
> sub-expression (round 1), an unexpanded variable inside a well-formed argument (round 2), and a body piped
> into the client with no body argument at all (round 3). Each fix asked "did I recognise this body channel?",
> so each left the next unlisted channel open. **§4.5 now states a whitelist of command shapes instead**
> (gate 1), which cannot be defeated by an unfamiliar channel — an unfamiliar shape simply fails. A standing
> rule says that when a third fix is needed, stop patching and re-derive; that is what gate 1 is.
>
> **THIS DOCUMENT IS THEREFORE NOT CONVERGED.** Gate 1 was written after the cap round closed and **no
> reviewer has seen it** — the same state v6.3 was in, and the reason v7 deleted v6.3's unreviewed residue.
> It is smaller than what it replaces and it closes all three measured shapes, but it carries none of the
> panel's endorsement. **A round 4 on gate 1 alone is the cheapest next step** and should happen before any
> code is written from this section.
>
...
outcome.** Three successive `/check` rounds each found a different shape in which the body was not covered
and the write was not refused, so this is stated as **a whitelist of command shapes, not an enumeration of
body channels** — the from-scratch re-derivation the standing "third fix" rule demands, rather than a fourth
patch.

**Gate 1 — the command shape must be on the whitelist.** A detected write qualifies only if it is a
**single client invocation**: exactly one HTTP-client or interpreter token; **no pipe into it**; no stdin
redirection (`<`, or a `-` / `@-` path standing for stdin); and no inline interpreter (`node -e`,
`python -c`, `pwsh -Command`, `-EncodedCommand`). Anything else is `body-source-unrecognised`.

Why a whitelist replaced the per-channel rule, stated because it is the whole lesson of the three rounds:
each earlier version asked "did I recognise this body argument?", so every body channel nobody had thought
of yet hashed command-only and passed. The round-3 case is `Get-Content -Raw body.json | Invoke-RestMethod
-Uri … -Method Patch` — `-Body` accepts pipeline input, so there is no body *argument* to classify at all,
and `scanChain` returns `NO-CHAIN` because **a single `|` is deliberately not a separator**
(`auto-approve.js:409`–`412`, read 2026-08-04). Asking "is this command shaped like one thing I can read in
full?" cannot be defeated by an unlisted channel: an unfamiliar shape fails the whitelist.
...
A bodyless write — `DELETE /v1/pages/{id}` with no body at all — passes gate 1 and takes the ordinary record
path. The whitelist is triggered by an unreadable *shape*, never by the mere absence of a body.
...
| `body-source-unrecognised` `[v7]` | yes | **reissue as a single client invocation with a literal body-file path** (§4.5 gates 1 and 2). Also not a record path |
