...
> `[v4]` **Superseded by `design-scoping-v3.md` §2/§3** for the stage mechanics and budgets. The
> ordering principle below survives and is strengthened by the network invariant in "A blocking
> decision must never wait on the network".
...
> `[v4]` **Superseded by `design-scoping-v3.md` §3.** Where the two disagree, that document wins.
> Note also the invariant added in "A blocking decision must never wait on the network", which
> moves the cold-cache case off the resolver entirely.
...
2. Consult the on-disk cache at **`~/.claude-staging/notion-page-parents.json`**.
   ...
   **the cache may only ever cache a fact that INCREASES gating; any fact that would DECREASE
   ...
   `[v4]` **What "advisory" does and does not mean, since v3's wording was self-cancelling.** v3
   said negatives "must not short-circuit a decision on their own" and in the same breath that their
   TTL "bounds a *performance* window" — but a negative that may never be acted on buys no
   performance, so the two halves cancel. The coherent reading, and the one the design takes: a
   cached negative **is** consulted, and it is allowed to route a call to the *approve* side, because
   the invariant that matters is the one in "A blocking decision must never wait on the network" —
   the approve side is where latency is safe and where a wrong answer costs over-gating on the next
   pass, not a silent hole. What a negative may never do is satisfy an *exemption that removes
   gating from a path that would otherwise block* — concretely the GEN-58 subtree flag, which is
   re-derived rather than trusted, and move-seeding, which purges rather than rewrites. So the
   principle stands as stated for exemptions; the 24-hour TTL is a real performance window for the
   ordinary non-ticket-page case; and the disclosed residual is unchanged and bounded: a page moved
   into Team-Tasks by someone other than Claude can read as out-of-scope for up to 24 h.
...
**So do not bet the design on the answer. Make it an invariant instead:**

> **No code path that ends in a block may depend on a network round-trip.**

That is achievable because of an asymmetry: a slow path ending in *approve* is harmless — if the
race exists, the call was going to run regardless. Only a slow path ending in *block* can silently
fail. The resolver exists solely to *exempt* non-ticket pages, so it belongs on the approve side.
Restructure accordingly:

- cheap in-payload tests (no network) -> out of scope -> return, fast.
- record lookup and verification -> all local disk -> approve, fast.
- no record, and membership known from cache -> block, fast. Invariant holds.
- **no record, and membership unknown (cold cache)** -> this is the only case v3 resolved over the
  network *before blocking*, and the only one that violates the invariant. Block immediately without
  resolving, and warm the cache out of band so the retry decides instantly and correctly.

The over-gating this introduces is one spurious block per cold page, self-healing on retry — a
strictly better failure than a silent hole, and a smaller cost than v3's already-accepted
"unknown -> hard-block" (Erez answered "go" to exactly that question on 2026-08-02).

**Which warming mechanism is an open build decision, not settled here.** Two candidates: (i) a
PostToolUse arm that resolves and caches from ordinary `notion-fetch` / `notion-update-page`
traffic — latency there is harmless, the pattern is already proven by `notion-fetch-staleness.js`
(fact 7), and the common flow self-warms because a ticket is nearly always fetched before it is
edited; cost is one more hook file. (ii) `/vet-ticket` resolves and writes the cache entry as part
of the run the block sends Claude to — no new hook, but it makes that escape need Notion access,
weakening the "an outage costs one local review" promise. (i) is better on both counts and worse on
machinery. Decide at build, with the invariant as the constraint either way.

**Blocking pre-build gate.** Before code is written against this design, test the race directly:
install the arm in the fixture tree with an artificial multi-second delay on the blocking path,
issue a real gated Notion write, and confirm the API call did **not** go out — not merely that the
hook script, invoked standalone, computed `exit 2`. If the race reproduces, the invariant above is
load-bearing rather than belt-and-braces, and any residual network-before-block path is a defect.
...
**v4 is that revision and has NOT yet been re-reviewed** as of this line being written; the panel's
second round is what decides convergence. The filename still says "converged" for continuity of
links — it refers to v2, and by the correction above, only partially to that.
