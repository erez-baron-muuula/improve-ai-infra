...
overrides `bypassPermissions` and the allow-list both — **verified live 2026-08-03**, not inferred:
`auto-approve.js` refused a chained Bash command of mine with its mixed-risk message and the command
did not execute. The three installed sibling gates block the same way in production.
...
- Of the 11 that are not: **4 are GEN-58 log volumes**, **3 are Team-Tasks rows the database query
  missed** (a Notion DB query omits archived rows and templates — a bias caught only by identifying
  all 11 individually; without that step the non-ticket share was overstated ~3×), **3 are corrupted
  or deleted ids**, and **1 is a genuine non-ticket page** — 1 reference in 1,081, **0.09%**.
...
**The housekeeping list, and the only place it is stated in this document.** Read from the live
schema (`GET /v1/databases/fe198002-6618-48d7-ae04-56f8cee479f3`, 2026-08-02). Full property set:
`Priority`, `Status`, `Urgency`, `Date Created`, `Assignee`, `Reason`, `Parent item`, `Attachment`,
`Project`, `Type`, `Text`, `Children`, `Gain ratio`, `Remind me (days before)`, `Due Date`, `ID`,
`Name`. **Housekeeping** = `Status`, `Assignee`, `Project`, `Type`, `Reason`, `Due Date`,
`Remind me (days before)`, `Date Created`, `ID`, `Parent item`. Everything else is substance —
including the title (`Name`), the three priority fields, and `Children`, the inverse-of-parent
relation whose write re-parents pages and which `notion-schema-guard`'s `(Parent item)`-suffix
matcher does not catch by name.

v3 stated this list twice, ten lines apart, and the two disagreed — one carried a `url` entry
matching no real property and omitted two fields Erez's decision explicitly exempts. **No other
section of this document may restate it.**
...
is not exactly 32 hex characters ignored, capped at 128 ids (beyond the cap the remainder is ignored
and the overflow logged).
...
file — and are **not** the authority. The token is. An `UPHELD` adjudication does not clear the gate.
...
6. Cheap pre-filter: `verdict === 'PASS' || waived === true`. Fails → hard-block.
7. Verify `reviewerAgentId` (and `adjudication`'s id, when present) against
   `<session dir>/subagents/agent-<id>.meta.json`: the sidecar must exist and its `agentType` must be
   `check-reviewer`. Fails → hard-block, consuming nothing.
8. Verify the verdict token per §6.2. Absent, wrong hash, file unreadable, or over the cap →
   hard-block. Skipped only when `waived === true`, where Erez's explicit chat answer is the
   authority.
...
4. **Branch on what the target actually is** — three lanes, because "every page is a ticket" gates
   ...
   - **A ticket create or edit** — the normal lane. Spawn a `check-reviewer` sub-agent that did not
     ...
     form.
   - **Not a ticket** — the target is an ordinary Notion page. The ticket bar does not apply, and the
     reviewer's brief says so explicitly: it confirms the target is not a Team-Tasks row and returns
     PASS with that as its finding. **This lane is why over-gating does not terminate on Erez** — a
     gap in v5 that three other findings leaned on for their escape. Cost: one sub-agent per
     non-ticket write, measured at ~1 edit in 1,081 and 1 create in 263. It deliberately adds **no
     new trust**: there is no "Claude declares this a non-ticket" shortcut, because that would be a
     hole rather than a lane.
...
6. **Evidence precondition, then write the record.** Verify: `reviewerAgentId` present; both
   ...
   `waived === true`; **and the reviewer's transcript actually carries
   `TICKET-REVIEW-VERDICT: PASS <hash>`** under §6.2's assistant-role rule — the skill runs the same
   check the hook will run, so a mismatch surfaces here with context instead of as a bare refusal
   later. Then write the record. The record write prompts nobody and **is not an approval step** — it
...
| exemption-file contents | a `/wrap` line: entry count, and any ids added since the last report | any id added by a route other than the appender is worth a look |
