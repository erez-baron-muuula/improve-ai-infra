...
>
> **Review state: five rounds, thirteen reviews — NOT converged. §4.5's gate 1 and gate 2 must not be built
> from; round 5 recommends replacing the mechanism, and that choice is Erez's. See the box directly below.**
...
>
> **Status of gate 1 and gate 2: DO NOT BUILD FROM THEM.** Five rounds, five instances of one class, and the
> panel's own reading is that the mechanism — not its rule — is what needs replacing. Per the standing
> stop-and-re-derive rule this text is **not** being patched a sixth time; the mechanism choice goes to Erez.
> Logged to GEN-58 as a Class E element, including that round 3's write-up claimed the stop-and-re-derive rule
...
1. **Drop the HTTP-client-name requirement.** `notion-schema-guard.js:131`/`:161` require the command to
   ...
   signal alone plus `api.notion.com` is enough. **What this buys is a refusal, not a review** — round 4
   corrected the earlier wording here. A client invoked through a variable or an unusual path
   (`& $curlBin -X POST …`) is now *detected* rather than invisible, and then **refused** by gate 1, which
   requires a recognised client token. Detected-and-refused beats undetected; it is just not additional
   reviewable coverage, which is what this bullet first implied.
...
outcome** — and that holds **only because of gate 1's closed shape below.** Four successive `/check` rounds
each found a different way a body escaped the hash while the write went through, and the first three fixes
were all enumerations of channels. The claim in this paragraph was false under every one of them; it is true
under a rule where an unrecognised token cannot pass.

**Gate 1 — every top-level token must be accounted for. This is a closed shape, in the same form §4.2
clause 1 and §5.1 clause 5 already use** — the only shape in this design that is safe against something
nobody enumerated. A detected write qualifies only if, reading **unquoted text only** (`scanChain` already
computes exactly that as `bare`, `auto-approve.js:407`), every top-level token is one of:

1. **the single client or interpreter token** — exactly one, and it must be a *recognised* client name. A
   client named through a variable (`& $curlBin …`) yields zero recognised tokens and therefore refuses;
2. **the URL literal**, containing no unescaped `$` or backtick — so the reviewer can see *which page* is
   being written, not just what is being written;
3. **a body-bearing flag from gate 2's table**, followed by an argument that qualifies in gate 2's sense;
4. **a flag on a closed list of body-less flags**, each with its own argument where it takes one:
   `-H`/`--header`, `-s`, `-k`, `-sk`, `-X`/`--request`, `-o`/`--output`, `--max-time`, `-Uri`, `-Method`,
   `-Headers`, `-ContentType`, `-OutFile`, `-UseBasicParsing`.

**Anything else refuses** — an unknown flag, a bare positional token, a `@name` splat, a lone variable — with
its own reason, **`body-shape-unrecognised`**, kept distinct from gate 2's `body-source-unrecognised` because
the two escapes differ and §10's monitor has to be able to say which gate is over-gating.

Three structural clauses are kept alongside the token rule, because they are about the command's shape rather
than its tokens: **no pipe into the client**; no stdin redirection (`<`, or a `-` / `@-` path standing for
stdin); no inline interpreter (`node -e`, `python -c`, `pwsh -Command`, `-EncodedCommand`).

**Why the token rule, and not the four-clause version this replaces — the fourth instance of one class.**
Rounds 1–3 each found a body channel the classifier did not recognise, so v7's first answer was "reject
unreadable *shapes*" and named four of them (multiple clients, a pipe, stdin, an inline interpreter). **Both
round-4 lenses independently found that this was still an enumeration**, one level up: anything avoiding
those four passed gate 1, and gate 2 only inspects flags on its own table, so a body arriving through
`--data-ascii "@file"` (a real curl alias absent from that table), `-K`/`--config` (which loads *both* flags
and body from a file), `-F`/`--form`, or a PowerShell `@props` splat was hashed as **bodyless** — and a
record was minted over bytes no reviewer saw. Worse than ungated: falsely assured. Detection does not even
depend on a body flag, since `EXPLICIT_WRITE_VERB_RE` (`notion-schema-guard.js:138`) fires on `-X PATCH`
alone.

Both lenses proposed the same fix, which is the one above, and it is different **in kind**: a token the hook
does not recognise cannot pass, so the rule is closed by construction rather than open by omission. That is
also why it belongs to this document's existing family of closed shapes rather than being a new idea. This is
the second time a "structural" fix of mine turned out to be an enumeration one level up.

> **`[round 5 — do not build from this section]`** Round 5 found that this rule's stated substrate does not
> exist on the path it runs on and deletes the bytes it must read (`bare` is returned only on `CHAINED`,
> `auto-approve.js:459`–`:460`, and erases quoted spans, `:424`–`:448`); that the closure claim below is false
> against curl's ambient config file; that gate 2's table mis-assigns `@path` semantics to `-Body` and
> `--data-raw`; and that the token rule refuses this project's ordinary write idioms while the cost sentence
> below understates it and contradicts §7.1. It also found that **parsing the command is the wrong mechanism**:
> `auto-approve.js` already handles this class by recognising a **named invocation form** (`:1317`, `:1322`),
> and an anchored whole-command template deletes six of the seven components specified here. Per the standing
> stop-and-re-derive rule this text is not being patched a sixth time — the mechanism choice is Erez's. The
> full round-5 record is in the review-state box at the top of this document.

**The cost, stated rather than discovered later:** an unusual but legitimate flag now over-gates at one
`/vet-ticket` run, and the body-less list will need extending the first time a real command uses something
absent from it. §10's raw-REST-blocks-by-reason row is what surfaces that, and `body-shape-unrecognised`
existing separately is what makes the row actionable.

This also settles the inline-interpreter family that widening 2 brings into scope. Those commands have no
CLI body flag by construction, so they are **detected and refused**, not gated-with-review. The coverage
claim is therefore precise: a native-call Notion write is *stopped*, not reviewed, and the escape is to
reissue it through a client with a literal body file, or through the MCP tool.

**A bodyless write** — `DELETE /v1/pages/{id}` with no body at all — takes the ordinary record path, and
`bodies: []` is correct for it **because every one of its tokens was recognised**, not because no body flag
happened to appear. That distinction is the whole finding above: the earlier wording read "no listed body
flag" as "no body", which is what let an unlisted channel through.

**Gate 2 — every body-bearing flag's argument must be classifiable.** For each such flag:

| flag | argument convention | hashed as |
|---|---|---|
| `-d`, `--data`, `--data-raw`, `--json`, `--data-binary` | `@<path>` → a file; otherwise a literal | file contents, or the literal text |
| `--data-urlencode` | as above, **plus** curl's `name@filename` form → a file | file contents, or the literal text |
| `-T`, `--upload-file` (curl), `-InFile` (PowerShell) | **a bare path, never `@`-prefixed** | file contents |
| `-Body` (PowerShell) | a literal, **or** `@<path>` | literal text or file contents |

**A qualifying argument is:** a single-quoted literal; a double-quoted literal with **no unescaped `$` or
backtick**; a **bare unquoted token** containing no `$`, backtick, quote or whitespace (this is the form the
third row above requires); or `@` followed by a path meeting the same test. **Anything else is
`body-source-unrecognised`** — including `@` followed by a variable, and including a parenthesised
sub-expression.

The three shapes that forced each part of this, one per round, recorded because the sequence is the argument
for gate 1:

- **Round 1 — a parenthesised sub-expression.** `-Body (Get-Content -Raw ticket.json)`: no `@`, no `$`, and
  `scanChain` returns `NO-CHAIN`, so nothing else would have caught it.
- **Round 2 — an unexpanded variable inside a well-formed argument.** `-Body "$body"`, `-d "$TICKET_BODY"`,
  `--data-binary "@$bodyFile"`: the argument *looks* like a literal or an `@path`, so a shape-only classifier
  hashes the **unexpanded** text while Notion receives the interpolated value. `scanChain` flags `$(` and
  `${`, never a bare `$name` (`auto-approve.js:409`–`412`).
- **Round 3 — no body argument at all**, handled by gate 1 above.

Also on this gate: a **uniform** "`@<path>` after a data flag" rule, which the first draft stated, would
never have recognised `-T body.json` or `-InFile body.json`, and would have hashed those command-only —
hence the per-flag table rather than one rule.

The refusal text names the fix in every case: **reissue the write as a single client invocation whose body is
a literal file path** (`--data-binary "@C:/…/body.json"`). Deterministic, and one substitution from the form
a caller would otherwise write.

**Body-file paths resolve against the PreToolUse payload's `cwd`, and a relative path that does not resolve
there is `body-file-unreadable`.** Left unspecified, a relative `@body.json` would resolve against the
command's runtime directory at execution and against whatever root the hook happened to use at hash time —
so a same-named file under the hook's root would have been hashed instead of the real body. A missing,
unreadable, or over-cap file (shared 2 MB) is the same hard block; nothing truncates.

**This does not conflict with the project's existing idiom, and the distinction matters.**
`scripts/notion-ticket-lookup.ps1` uses exactly `--data-binary "@$bodyFile"` (the call spans `:32`–`:36`,
with that flag at `:36`) — but it is a **read** (`POST …/query`, on the read allow-list), so this arm never
sees it. The literal-path requirement applies only to gated *writes*.
...
(`scripts/notion-ticket-lookup.ps1:33`). This is a refusal in the skill, not the hook: the hook must not
...
installed gate unusable in daily work". The honest guarantee after v7 is stated once, normatively, in §14's
coverage bullet — **not** here, and not in §12.1. Both previously carried the looser form "named in a shell
command", dropping §14's qualifier "whose body is resolvable from the command text"; a round-5 advisory
caught it, and it mattered because §12.1's wording is the one §12.1 instructs writing into the GEN-508
ticket. Either way it is materially wider than v6.3's "through the MCP tools", and not the same thing as
"every write". Piece 2 owns the script-file case; §10 carries the counter that says whether it is costing
anything.
...
record with no hash binds nothing. **`--ticket-hash-shell` adds one more non-zero condition** `[v7]`: the
command fails §4.5's gate 1 or gate 2, i.e. its body cannot be bound. Without that, the CLI would hand back
a hash for a command the arm will refuse to hash at all, and a reviewer run would be spent before the
refusal text redirected the caller (round-3 advisory).
...
3s. **Shell path** `[v7]`. Run the §4.5 detector. Not a detected Notion write → return, untouched.
   ...
   shape; exempt → return. Then **§4.5 gate 1** — every top-level token accounted for, plus the three shape
   clauses — hard-blocking on `body-shape-unrecognised`; then **gate 2**, hard-blocking on
   `body-source-unrecognised` (a body argument that is neither a literal nor a resolvable file path) or
   `body-file-unreadable` (missing, unreadable, or over the 2 MB cap). Only then compute the
   `{shell, bodies}` hash, and continue at step 4 — every step from here on is shared with the MCP path,
...
4. **Read the record directory for `block` verdicts too, not only `in`.** Every `scope:'block'`
   ...
   fallback, every block **on the MCP path** carries a hash a record can match. **The shell path's three
   pre-hash blocks are the stated exception** (§7.1): they fire at step 3s before any hash exists, and their
   escape is to reissue the command, not to obtain a record.
...
| `rest-chained` / `rest-chain-unreadable` `[v7]` | yes | **reissue the write as its own single, single-line command** — not a `/vet-ticket` run; no record can clear these, because they block before any hash exists (§7 step 3s) |
| `body-shape-unrecognised` `[v7]` — gate 1 | yes | **reissue with every token recognised**: one named client, the URL, and only flags on §4.5's two lists. A `@splat`, a `-K`/`--config`, an unknown flag or a bare positional token is what fired it. Not a record path |
| `body-source-unrecognised` `[v7]` — gate 2 | yes | **reissue with the body as a literal file path** (`--data-binary "@C:/…/body.json"`). Not a record path |
| `body-file-unreadable` `[v7]` | yes | fix the path (it resolves against the payload `cwd`) or the file, then reissue |
| **shared normalisation / hashing bug** | yes | **global break-glass only** |

**Three rows here escape by rewriting the command rather than by obtaining a record**, and that is a
deliberate exception to §7 step 4's rule that every block carries a hash a record can match: on the shell
path those three fire *before* a hash exists, so step 4's guarantee is scoped to the MCP path and to
`unreadable-payload`. A round-3 lens found both this table and that sentence asserting completeness they no
longer had. The last row remains the only class whose escape is break-glass, and v3 implied it did not exist. `/vet-ticket` gets its
...
`rest-chain-unreadable`, `body-shape-unrecognised` (gate 1), `body-source-unrecognised` (gate 2),
...
| **raw-REST blocks, by reason** `[v7]` | a `/wrap` line | `rest-chained` / `rest-chain-unreadable` / `body-shape-unrecognised` / `body-source-unrecognised` / `body-file-unreadable` recurring across two windows means §4.5 is over-gating real work — and the reason says which gate to narrow: `body-shape-unrecognised` means gate 1's body-less flag list is too short (extend the list), `body-source-unrecognised` means a caller is passing bodies in a form gate 2 cannot bind (fix the caller) — narrow the shape, do not disable the arm. This is the monitor the arm is held to for the same reason §12.1 demanded one of every option |
...
GEN-508 piece 1 delivers **every Notion create/edit reachable through the four MCP write tools, or named in
a shell command whose body is resolvable from the command text**. **Copy that sentence from §14's coverage
bullet, which is the single normative statement of it** — a round-5 advisory found this paragraph and §4.5
both carrying a looser form that dropped the body-resolvability qualifier, and this is the paragraph that
...
lives only inside a script file the command runs is invisible to it, and §4.5 explains why closing that
particular gap would gate routine reads. §12.2 clause 4 holds the same requirement for the rule-coverage half.
...
- **Coverage after v7, in one sentence, because the ticket's title says "every"** `[v7]`: every Notion
  create/edit reachable through the four MCP write tools **or named in a shell command whose body is
  resolvable from the command text**. Two cases sit outside it: a write whose URL appears only inside a script
  file the command runs (§4.5 records why the obvious widening must not be adopted — it would gate routine
  Notion *reads* performed by an existing script), and a write whose body cannot be read from the command,
  which is **detected and refused** rather than reviewed. Refused is a safe outcome but it is not coverage, and
  round 4 was right that the sentence has to say so. The rule-coverage half of "every" is §12.2's, and it is a
  separate claim with its own acceptance criteria.
