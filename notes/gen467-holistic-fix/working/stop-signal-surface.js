 ...
 *
 * GEN-467 etiquette (REWRITTEN 2026-08-09, GEN-467 holistic fix Part 1; design:
 * Improve AI Infra/notes/gen467-holistic-fix/design-v4r3.md). This note is
 * MESSAGE-SPECIFIC, so it embeds its block instruction rather than deferring on
 * the block marker -- but the instruction is now STATE-AWARE. The previous text
 * unconditionally said to fold the design "into the block this turn owes", plus a
 * prohibition scoped to "merely in response to this note"; on 2026-08-03 that
 * wording fired at a Stop whose message ALREADY carried the block and produced a
 ...
 * the fold-in instruction with the dual-branch tail. Keep this note's text in
 * sync with the global signal-surfacing rule, and run stop-claim-linter.js's
 * reason-self-scan fixture when editing either branch (neither may match a
 * CLAIM_PATTERNS / SELF_AUDIT_PATTERNS entry or contain a line-start opener).
 *
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // `decision` field records what this hook DECIDED, not that a nudge was confirmed
    // delivered -- a stdout write can still fail afterwards (fail-open). /wrap uses
    ...
    // Both branches share the diagnosis; they differ ONLY in where the designed
    // surfacing path is to be DELIVERED. Fixture constraint (stop-claim-linter.js
    // reason-self-scan): neither branch may match a CLAIM_PATTERNS /
    // SELF_AUDIT_PATTERNS entry ("no block is due" is the deliberate near-miss;
    // never "owed") or contain a line-start opener.
    const common =
    ...
    if (blockAppearsSent) {
      additionalContext = common +
        ', and deliver it like this: the finished reply appears to already carry this ' +
        'turn\'s "\u{1F4CC} For you" block. If that matches what you sent -- the block ' +
        'went out -- state the surfacing design briefly as plain prose and do not emit ' +
