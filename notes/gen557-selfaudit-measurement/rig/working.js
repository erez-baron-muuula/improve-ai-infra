 ...
 * errors. NOTE (Phase 2): a CLAIM_PATTERNS/SELF_AUDIT_PATTERNS false positive on
...
const SELF_AUDIT_PATTERNS = [
  ...
  //    stage -- see the Phase-2 note). They add 113 production fires, ALL 113
  //    inspected, 0 false. Rig + per-pattern evidence: GEN-557.
  //    `isNew: true` marks the GEN-557 additions. It scopes the isQuoted() guard
  //    below to THESE patterns only, leaving the 10 shipped ones behaving exactly
  //    as they do today -- this ticket widens coverage, it does not re-tune already
  //    vetted patterns. (Widening the guard to all 16 was measured: it would remove
  //    4 more false nudges and wrongly suppress none, but it is a separate decision
  //    about shipped behaviour, so it is not taken here.)
  ...
  //    NOTE: being line-start-anchored, this pattern gets NO protection from
  //    isQuoted() by construction -- there is never any same-line text before the
  //    match to count quotes in. A fenced or block-quoted EXAMPLE of the roll-call
  //    shape will therefore fire. Fail-open, consistent with this stage's stance.
...
function findSelfAudit(text) {
  ...
  for (const pat of SELF_AUDIT_PATTERNS) {
    ...
    while ((m = re.exec(text)) !== null) {
      ...
      // Gated on pat.isNew: the 10 pre-GEN-557 patterns keep their shipped behaviour.
      ...
      const hit = m[0].trim().slice(0, SELF_AUDIT_MAX_HIT); // hard cap (see const)
