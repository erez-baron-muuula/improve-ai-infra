...
//
// RESIDUALS -- every one leaves a MISSED nudge, never a wedge or a forced redo:
//  - Cannot tell quoting-to-discuss from quoting-to-say, so scare-quoted or inline-code'd
//    narration escapes: `It "came back clean" on both files.` The corpus evidence below
//    bounds only shapes that HAVE occurred; it says nothing about that one, which is one
//    keystroke away. Do not read the measurement as covering it.
//  - Parity is per LINE, so a multi-line quoted block is not covered, a ``` fenced block
//    is not covered (see above), and single-quoted spans are not covered. Every one of
//    those leaves the pre-guard behaviour: a fire.
//  - Suppression is tested at the MATCH START only, so a match straddling a quoted span
//    is judged by the side it starts on.
