...
// explicitly on 2026-08-02 (the narrow `pat.isNew`-gated alternative was built and
// offered alongside it).
// What the widening does, measured on 4,356 real turn-final messages: applied to the
// shipped patterns IN ISOLATION it changes 6 turns and fully silences 4 -- all 4
// inspected, all 4 genuine quoted references (the phrase under discussion, e.g. 'The
// "Nothing else was touched" line was backed by...'), with no genuine hit wrongly
// suppressed anywhere in the corpus, so the false-negative direction is measured
// rather than assumed. Shipped TOGETHER with the six patterns above the net is 2, not
// 4, because 2 of those 4 messages also say "Nothing to correct" and so still fire on
// a new pattern: 190 -> 188 fires.
// MAINTENANCE: a pattern added later is covered by this guard automatically. If one
// ever needs to opt OUT, gate the call site on a per-pattern flag rather than
// weakening the test here.
