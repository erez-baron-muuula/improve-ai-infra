...
// hook with no timeout). The mask below is one linear pass, then O(1) per match: 15.9ms
// on that same input, vs 23.3ms for the pre-change file, because a masked match now
// short-circuits BEFORE the vicinity slice and the clearing regexes. Per-match cost is
// therefore never above the pre-change cost -- the worst case is an input where nothing
// is masked, measured at 90ms for ~22k unsuppressed matches in 200k.
...
// FENCE PARITY, both directions of it: an ODD number of line-start ``` markers -- an
// unterminated fence, or a fenced example that itself contains a fence opener -- masks
// everything to end of message, silencing every pattern there. Missed nudge, never a
// wedge; the same parity fragility is already a disclosed residual for stripFences on
// the blocking arm, where the stakes are higher. One consequence worth naming: self-
// audit narration inside a fenced DRAFT of a "For you" block now escapes, and that is
// the only reachable route to block-internal narration, since a recognised
// block-carrying message is never scanned at all. Suppressing a draft is the intended
// call; measured cost on 4,356 real messages was zero changed outcomes.
// NOT masked, deliberately worth stating: the claim-linter's own note text contains the
// literal "no block is owed", which matches a pre-GEN-557 pattern -- a standing
// violation of this file's own guard-reason fixture. Straight-quote parity before that
// phrase is always even, so this mask does NOT suppress it and the violation still
// surfaces when the fixture is run. It is pre-existing and filed separately; the mask
// neither caused nor hid it. (Pass B round 2.)
