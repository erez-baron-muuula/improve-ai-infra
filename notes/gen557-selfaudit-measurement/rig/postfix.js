...
const cases = {
  ...
  // Pass B round 2, advisory (c): the shape the earlier timings did NOT cover --
  // ~22k matches that are all UNSUPPRESSED, so every one pays the full vicinity +
  // clearing cost rather than being pre-filtered away.
  '200k of unsuppressed matches (`✓ closed ` x ~22k)': '✓ closed '.repeat(23000).slice(0, 200000),
  '200k unsuppressed, pre-existing pattern (`came back clean `)': 'came back clean '.repeat(13000).slice(0, 200000),
};
...
// NOTE (2026-08-02): fence handling was built, reviewed, and then DROPPED on Erez's
// call -- unbalanced fence markers silenced whole messages. So the two "fenced ...
// must stay silent" cases below are now expected to FIRE; they are kept, with flipped
// expectations, as the record of what the drop costs. maskprobe.js is the fuller
// post-drop fence suite. Open follow-up: GEN-592.
console.log('\n=== Finding 2 (SUPERSEDED by the fence drop): fenced examples now fire ===');
...
for (const [label, t, want] of [
  ['fenced roll-call example (residual: fires)', fencedRollcall, 'FIRE'],
  ['fenced copy of the whole must-FIRE fixture (residual: fires)', fencedWhole, 'FIRE'],
