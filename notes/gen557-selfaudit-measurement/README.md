...
**Still owed: a Pass A re-run on the current diff.** Rounds so far: Pass A r1 (8 findings) → Pass B
r1 (1 material) → fixes → Pass B r2 (Finding 1 RESOLVED, 2 new) → Pass A r2 (7 findings) → the
fence drop plus text fixes. `/vet-code` Step 3 allows one re-review round after non-trivial fixes
and then says stop rather than loop, which is why the fence-drop decision went to Erez. The drop
is behaviour-neutral on real traffic (fire counts identical: 77 → 188, +113/−2) and is covered by
`rig/maskprobe.js`, but the current diff has not been through a Pass A. Claude cannot run it —
Erez must type `/code-review` pointed at `rig/proposed.diff`.
