...
1. **One decision for Erez** — the quote guard's scope (see Finding 3 below). The working
   copy currently takes the narrow, in-scope default. Widening is measured and available
   but changes shipped behaviour, so it needs his explicit yes.
2. **`/code-review` must be typed by Erez.** It is configured `disable-model-invocation`,
   so **Claude cannot invoke it in any session** — not a desktop-only limitation, which is
   a sharper diagnosis than **GEN-546** records. `/vet-code` Step 0 fails closed without
   it. Remaining sequence: Erez answers the scope question → Erez types `/code-review`
...
| working copy (narrow guard scope — the default) | **190** (+113, −0) |
| working copy (guard widened to all 16 — needs Erez's yes) | **188** (+113, −2) |
...
## Open decision for Erez (blocks nothing else, but should settle before `/code-review`)

**Should the quote guard also cover the 10 patterns that shipped before GEN-557?**
- **No (current default).** Only the six new patterns get it. Shipped behaviour untouched.
  Fire count 190. The 4 known false nudges on shipped patterns stay.
- **Yes.** Fire count 188. Removes 4 measured false nudges (2 of them net, as above);
  measured to wrongly suppress nothing anywhere in the corpus. But it re-tunes patterns
  that were already vetted under a different ticket.
