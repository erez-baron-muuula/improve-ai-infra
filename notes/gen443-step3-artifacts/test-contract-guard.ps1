...
$saysExtra = $outS.Contains($msgExtraHeading) -and $outS.Contains("at entry line(s) 5")
$noDemoteOwn = -not $outS.Contains("at entry line(s) 2, 5")
Report "(s) displaced heading + a real extra: each named correctly in ONE message" (($rcS4 -eq 4) -and ($beforeS -eq $afterS) -and ($recS4 -eq 1) -and $saysDisplaced -and $saysExtra -and $noDemoteOwn) "rc=$rcS4 (want 4) untouched=$($beforeS -eq $afterS) recoveryFiles=$recS4 (want 1) displacedNamed=$saysDisplaced extraNamed=$saysExtra ownHeadingNotCalledExtra=$noDemoteOwn"
