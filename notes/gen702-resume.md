...
## Closeout (mandatory — so nothing lingers stale)
The moment GEN-702 ships and is verified and its ticket is set to Done, delete ALL THREE resume artifacts as a required completion step — do NOT leave them behind:
1. this file (`notes/gen702-resume.md`),
2. the memory file `~/.claude/projects/C--Users-Erez-AI-Projects-Improve-AI-Infra/memory/gen702-resume-active.md`,
3. its one-line pointer in that dir's `MEMORY.md`.
Do this on task completion, NOT on read (the cue must survive multiple `/compact`s until the work is actually done). Backstop: if this brief is still present on or after **2026-09-01** and GEN-702 is Done or abandoned, delete all three then.
