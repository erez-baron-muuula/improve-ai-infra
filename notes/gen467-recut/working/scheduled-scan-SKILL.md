...
Code prunes old transcripts, so do not let the window stretch to months). At
the END of a scan run, decide the next firing and create it in the same
session: bars all clear → schedule one more scan +14 days out; that second scan
also all clear → propose GEN-467 closure to Erez instead of a third scan; any
bar tripped → follow the bar's action and schedule per the reopened work. If at
firing time fewer than 5 post-apply block-carrying turns exist, do not judge —
reschedule +7 days and say so in the report.
...
each bar's verdict, the interim compliance lines, and — only if a bar tripped —
the recommended next step. A clean scan reports the numbers and "all bars
clear" briefly; it does not narrate the checking.
