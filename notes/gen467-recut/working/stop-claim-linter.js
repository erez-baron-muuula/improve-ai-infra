 ...
 *  harness's 8-in-a-row override; at cap the guard released. The guard still
 *  deliberately does NOT early-exit on stop_hook_active (the legitimate block
 *  rides the nudge continuation where that flag is true; probe-verified).
 *
 ...
 *  first blocks write no release record). The live health signal is the
 *  PRESENCE of phase1-skip-blockform / suppressed-block-out rows alongside a
 *  duplicate-free transcript census -- see that scan's SKILL.md (shipped with
 *  this batch; working copy in notes/gen467-recut/) for the bars. If the task
 *  text is missing the guard-log section, restore it from that SKILL.md.
 *
